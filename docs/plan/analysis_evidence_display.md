# 解析の記録・表示の改善（確認事項 A・B・D）実装方針

対象: `docs/reports/observability-qa-2026-08-07.md`（顧客からの確認事項）のうち、
**確認事項 A：表示範囲**（A-1 / A-2 / A-3）、**確認事項 B：記録範囲**（B-1 / B-3 / B-4）、
**確認事項 D：表示の不整合**（D-1 / D-2 / D-3）。
B-2 と確認事項 C は未着手（末尾「積み残し」参照）。
...
- **表示**: 「実行履歴」タブは現在 UI 上で非表示のため、顧客が実際に見る**解析履歴タブの表**に
  **成功した解析と同じ表へ日時順に混ぜて**表示する（`status=failed` で取得）。失敗行は結果が
  無いため詳細ページへは遷移させず（クリック不可）、淡い赤背景と結果バッジで区別する。
  実行履歴タブ側にも結果列・フィルタ・詳細を追加済み（再表示した場合用）。
- テストが開発用 DB を汚さないよう `tests/conftest.py` で SQLite を一時ディレクトリに隔離した
  （400/422 を検証する既存テストが実 DB に書き込むようになったため）。

## 5. 確認事項 D への対応（D-1 / D-2 / D-3）

### D-3: schema_version の v0.1 固定（更新漏れ）

`schema.py` に **`SCHEMA_VERSION = "v0.2"`** を定義し、唯一の定義元にした。
`AnalysisResult.schema_version` の既定値と、**5 構成すべて**（config1/2/3/4/5）の
Langfuse trace metadata がこれを参照する。

v0.2 へ上げた際に metadata 側だけ更新漏れが起きたのは、5 箇所にリテラルが
散っていたためなので、**リテラル禁止のテスト**（`"schema_version": "v..."` の
grep）を追加して再発を防ぐ。既存トレースの metadata は書き換わらない（新規実行のみ）。

### D-1: 委譲チェーン履歴の「上限 0」

`rally_agent` は `delegation_max_rounds` を正しく入れているが、config-log 解析は
`rally_two_stage._build_final_result` で結果を組み直す際に引き継がず 0 になっていた
（1 段階モードも同じ経路を通るため全モードで発生）。

- `StageOutput.delegation_max_rounds` を追加し、`_result_to_stage_output` →
  `_build_final_result` と伝播（`delegation_history` と同じ扱い）
- UI の `StageResultView` が組み立てるダミー result も Stage の値を使う（Stage タブでも正しい上限）
- ユーザー延長時は**延長後の最終値**が入る

### D-2: metrics.cost_usd の実計算

`cost_usd` は常に 0.0 で「当てにならない」値だった。コスト計算は Langfuse 送信用の
`usage_for()` にしか無かったため、`tracing.cost_usd()` として切り出して共有する
（prompt caching の倍率: 書込 ×1.25 / 読出 ×0.1 を含む同一計算）。

**決定事項**:

| 論点 | 決定 | 理由 |
|---|---|---|
| `cost_usd` の範囲 | **本解析のみ**（orchestrator + 監視 + integrator） | tokens と定義が揃う。既存履歴と同じ土俵で比較できる（A-2 と同じ判断） |
| プランナー・監査分 | UI で**別枠 + 合計**を併記 | 同上。監査コストが「どこにも計上されない」問題も解消 |
| キャッシュ内訳 | `RoundMetrics` に `cache_creation` / `cache_read` / `cost_usd` を**保存する** | 内訳が無いと後からコストを再計算できず（読出は 1/10 単価）、キャッシュ効果も見えない。入力トークン削減の効果測定に直結する |
| 単価未登録モデル | `info_loss_flags` に `cost_unpriced_models:` を残す | 0 を「無料」と誤読させない |
| 既存 213 件の履歴 | **0 のまま「—（対応前の解析）」表示** | キャッシュ内訳が無く正確な遡及計算ができない。上限値で概算すると実額より大幅に高く出る |

表示は結果ペインのサマリカード（本解析 / プランナー / 監査 / 合計）、ラウンド別
リソース消費のコスト列＋キャッシュ内訳、解析履歴詳細のメタ欄、Markdown レポート。
...
| 項目 | 判断 |
|---|---|
| **B-2**（発行 SQL と取得行の記録） | **対応不要**（2026-08-13 判断）。事後検証は GCP の BigQuery ジョブ履歴（クエリ履歴 / `INFORMATION_SCHEMA.JOBS`）で行える。アプリ側に取得行を保存すると機微データの保持量が増えるため、保存しない現状を維持する。A-3 でも `bq_evidence` は保存対象外としている |
| **C-1**（Langfuse Input の 2,000 字と log_compaction の区別） | 仕様の明確化のみで改修不要。報告書での説明で完了 |

これで確認事項 A・B・D はすべて対応済み（B-2 を除く）。
