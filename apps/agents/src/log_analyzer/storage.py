...
``overrides_json`` は ``{slot_id: 上書きプロンプト}``、``model_overrides_json`` は
``{slot_id: 上書きモデル名}`` を JSON 化した文字列。
...
def init_db() -> None:
    """テーブルが無ければ作成（idempotent）。

    Phase 2 W6 で ``model_overrides_json`` カラムを追加。古いスキーマ
    （カラム欠落）が残っていた場合のみ DROP して再作成する条件付きマイグレーション。
    新しいスキーマで動いているインスタンスでは保存データは保持される。
    """
    _ensure_dir()
    with _connect() as conn:
        ...
        if cols and "model_overrides_json" not in cols:
            # 旧スキーマ → DROP して再作成（PoC 期間中の互換性は保証しない）
...
_COLS = "id, name, base_config, overrides_json, model_overrides_json, created_at, updated_at"
...
def update_saved_config(
    ...
    with _connect() as conn:
        cursor = conn.execute(
            "UPDATE saved_configs SET overrides_json = ?, model_overrides_json = ?, updated_at = ? "
            "WHERE id = ?",
            (p, m, now, config_id),
        )
        conn.commit()
